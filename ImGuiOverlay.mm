...
- (instancetype)init {
    if ((self = [super init])) {
        ...
        // Place button at top-right with some padding
...
- (void)setupWithDevice:(id<MTLDevice>)device
    ...
    io.FontGlobalScale = 1.5f; // Larger text on phone

    // 2. Dark theme with custom accent colour
    [self _applyDarkTheme];

    // 3. Init Metal backend
...
- (void)beginFrameWithCommandBuffer:(id<MTLCommandBuffer>)commandBuffer
    ...
    // FPS accumulation (update once per second)
...
- (BOOL)_processTouches:(NSSet<UITouch *> *)touches phase:(UITouchPhase)phase {
    ...
    if (phase == UITouchPhaseBegan)                  io.MouseDown[0] = true;
    else if (phase == UITouchPhaseEnded ||
             phase == UITouchPhaseCancelled)          io.MouseDown[0] = false;
...
- (void)_buildUI {
    ...
    // Draggable — track drag via ImGui
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_FrameRounding, btnSz.x / 2.f); // circle
    ...
    // ── Placeholder for user debug modules ────────────────────────────────────
...
- (void)_applyDarkTheme {
    ...
    c[ImGuiCol_ScrollbarBg]         = ImVec4(0.02f, 0.02f, 0.04f, 0.00f);
}
